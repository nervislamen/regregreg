...
if(!kullanici) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Bir @murphy/İD girmelisin..`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
...
message.channel.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('0x348f36').setTimestamp().setDescription(`${cliet.config.susturulduKalkti} ${message.author} tarafından ${kullanici} \`${sebep}\` seslide susturulması bitirildi.`));
