...
message.channel.send(`${client.config.murphyTag2} <@${message.author.id}> Kullanıcı Adın Değiştirildi.`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })) 
boosterisimlog.send(`${client.config.onayemoji}> **${kullanici.user.tag}** Adlı Kullanıcı Boost Bastığı İçin İsmini Değiştirdi.`);
}
