...
if(!kullanici) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Bir @Murphy/İD girmelisin`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
...
message.channel.send(` ${client.config.Cezalandrld}> **${kullanici.user.tag}** adlı kullanıcı __${sebep}__ Sebebiyle **${message.author.tag}** Tarafından yasaklandı.`) 
