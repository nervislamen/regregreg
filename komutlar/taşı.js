...
exports.run = (client, message, args) => {//
    ...
    return message.channel.send(new MessageEmbed().setDescription(`${message.author} Komutu kullanmak için yetkin bulunmamakta.`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
          ...
          if (!kullanici) return message.channel.send(new MessageEmbed().setDescription("Taşıyacagın Kişinin @murphy/İD'sini girmelisin.")).then(x => x.delete({timeout: 5000}))
    ...
    kullanici.voice.setChannel(`${kanal}`)
        .then(() =>
            message.channel.send(new MessageEmbed().setDescription(`${client.config.murphyTag} ${kullanici} **<#${kanal}>** adlı kanala taşındı.`))).then(x => x.delete({timeout: 5000}))
        .catch(console.error);
}
exports.conf = {
    enabled: true,
