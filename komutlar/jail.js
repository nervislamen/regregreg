...
exports.run = async (client, message, args) => {
 if (!message.member.roles.cache.has((client.config.jailhammer)) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.sendEmbed(new Discord.RichEmbed().setDescription('**Bu komutu kullanabilmek için** Jail Hammer **Yetkisine Sahip Olman Gerek**').setColor("Black"));
...
message.guild.members.cache.get(member.id).roles.remove(r)
  ...
  member.roles.add((client.config.jailrol))
    ...
    .setDescription(`${client.config.murphyTag} ${kullanıcı} Adlı Üye ${message.author} **Tarafından** **${reason}** Sebebi İle Hapishaneye Atıldı`)
    .setColor("RED") 
  ...
  message.react((client.config.onayemoji))
  .setDescription(`${client.config.Cezalandrld} ${kullanıcı} Adlı Üyeye <@&${client.config.jailrol}> Cezalı Rolu Verildi`) 
