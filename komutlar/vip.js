...
exports.run = async (client, message, args) => {
 ...
 if (!kullanıcı) return message.channel.send(new Discord.MessageEmbed().addField((client.config.sunucuadı) , `${client.config.murphyTag} Bir @murphy/İD girmelisin.`).setColor("2e0101").setFooter(message.author.tag ,message.author.avatarURL).setTimestamp());
  ...
  let embed = new Discord.MessageEmbed()
    ...
    .addField(
      (client.config.sunucuadı),
      `${client.config.murphyTag} ${member.user} **adlı üyeye <@&${client.config.vip}> rolu verildi**`)
