...
exports.run = async (client, message, args) => {
  const yetkilog = message.guild.channels.cache.find(c => c.id === (client.config.yetkilog)); 
    let embed = new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('#009dff').setTimestamp().setThumbnail(message.author.avatarURL).setFooter(client.config.footer);
        ...
        if (!member) return message.channel.send(embed.setDescription(`${client.config.murphyTag} Bir @murphy/İD Belirtmelisin.`))
        ...
        await message.guild.members.cache.get(member.id).roles.add(client.config.yetkiliunutmusuz)
...
exports.conf = {
    enabled: true,
    guildOnly: true,
    aliases: ["yetki-ver","yetkiliyap","yetkibaşlat","ybaşlat","yetki-başlat"],
    permLevel: 0,
    name: "yetkili-yap"
  }
